...
export default function RitualLogicFields({ state, set, accent, onUploadImage, onDownload }) {
  ...
  return (
    <div key="ritual-logic-fields">
      ...
      <div className="field"><label>Composition</label><div className="toggle">{['Chain', 'Split', 'Casefile'].map((variant) => <button key={variant} className={'seg' + (state.ritualVariant === variant ? ' sel' : '')} onClick={() => set({ ritualVariant: variant })}>{variant}</button>)}</div></div>
