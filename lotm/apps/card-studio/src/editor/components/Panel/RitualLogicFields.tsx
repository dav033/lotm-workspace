...
export default function RitualLogicFields({ state, set, accent, onUploadImage, onDownload }) {
  ...
  return (
    <div key="ritual-logic-fields">
      ...
      <div className="field"><label>Composition</label><div className="toggle ritual-composition-toggle">{['Chain', 'Split', 'Casefile', 'Pressure', 'Timeline'].map((variant) => <button type="button" key={variant} className={'seg' + (state.ritualVariant === variant ? ' sel' : '')} onClick={() => set({ ritualVariant: variant })}>{variant}</button>)}</div></div>
      ...
      <div className="field"><label>Ritual function</label><textarea rows={4} maxLength={360} value={state.ritualText ?? ''} onChange={(e) => set({ ritualText: e.target.value })} /></div>
      <div className="field"><label>Potion pressure</label><textarea rows={4} maxLength={360} value={state.ritualSurvival ?? ''} onChange={(e) => set({ ritualSurvival: e.target.value })} /></div>
