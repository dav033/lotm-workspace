...
- [x] P6.1 shared/ moves · [x] P6.2 server-only enforced · [x] P6.3 i18n removed · [x] P6.4 store slices *(`store.ts` 964 → 61 ln)* · [ ] P6.5 admin tree split + manual checklist *(pure layout, SVG primitives and views all extracted with tests; the three containers' orchestration and the checklist stay open)* · [x] P6.6 datos split + export→validate round-trip test *(DB-backed merge/replace import still pending: needs `TEST_DATABASE_URL`)* · [x] P6.7 diagnostico split · [x] P6.8 dead code sweep · [ ] **M4 merged**
...
- **2026-08-01 — audit, P7 gate:** Docker is not installed on the development machine, so neither image build nor `compose config` could be validated locally. Per §3.1 the M2 merge must be coordinated with the owner and verified on a machine with Docker. The game service no longer mounts `lotm_data`, which is intended since `data/` is now studio-only.

- **2026-08-02 — correction, `Dockerfile.game` CMD:** An earlier audit note flagged its `npm run start -w @lotm/game` under `WORKDIR /app/apps/game` as a possible failure. That was wrong. Reproducing the image layout (workspace root plus `apps/game` only, without `apps/card-studio` or `tools/`) shows npm walks up, finds the workspaces root and resolves the workspace correctly. No change needed.
...
- **2026-08-02 — P6.6 round-trip, partial:** The backup format is v5, not the v3 the step names. `datos/roundTrip.test.ts` now drives `exportarContenido` over a stub populated with every entity kind and feeds the result to `validarDocumento`, which is the contract the split could break silently. Writing it caught three real format rules the fixture violated (`chance` is 0–1, and advance and ritual formulas must total exactly two units). What still needs `TEST_DATABASE_URL` is the other half: running `importarContenido` in merge and replace modes against a scratch schema and comparing with the original.

- **2026-08-02 — test invocation:** `apps/game` runs its suite through `node --import tsx --loader ./test/server-only-loader.mjs`, which neutralises the `server-only` import that P6.2 added everywhere. Invoking `npx tsx --test` directly bypasses the loader and every test touching a server module fails with a misleading "cannot be imported from a Client Component" error. Noted in the game's `CLAUDE.md`.
...
- **2026-08-02 — P6.5 partial, and narrower than the line counts suggest:** The extractions this step prescribes are largely already in place. The pure layout lives in `arbol/disposicion.ts` (496 ln, with a fixture test), alongside `subgrafoFase.ts`, `agrupacionFases.ts`, `modeloResumenFases.ts` and `tipos.ts`, each with its own test; the SVG components live in `arbol/primitivas.tsx`; edge filtering lives in `arbol/layoutConexiones.ts` with a test. `ArbolConexiones.tsx` and `ExploradorArbol.tsx` already consume all of it. This session added the seven presentational components in the tail of `MapaFases.tsx` plus the shared `EstadoElemento`/`ESTADO_META` pair.

  What is left in all three files is container orchestration: state, effects, handlers and JSX assembly. Splitting that is gated on the manual browser checklist this step defines, and no automated UI test exists, so it stays open rather than becoming an unverifiable change to live admin tooling.

- **2026-08-02 — P5.4/P5.7 are cosmetic where it matters:** 21 files under `apps/card-studio/src/editor/` carry `// @ts-nocheck` — effectively the whole editor layer, including `EditorWorkspace.tsx`, `useEditorController.ts`, `useCardSession.ts`, `Filmstrip.tsx` and every `Panel/*Fields.tsx`. P5.7's "typecheck green with zero .js/.jsx left" is therefore true of the file extensions only: the files were renamed to `.tsx`/`.ts` and their contents exempted. Real typing of that layer is still open work.

- **2026-08-02 — runtime bug found by running the app (P5.2 regression):** `/cartas` was dead in the browser for every card family except Character and Artifact. `cardProps.ts` emits `accent` only on the standard family's props, but the editor tints its chrome with the open card's accent whatever the type, so `accent.c` threw. Fixed with `accentForState()`, which restores the original chain from `App.jsx`, plus `accent.test.ts` covering all twelve families. Worth recording how it escaped: lint, typecheck, tests, build and the PNG goldens were all green at the time — the goldens exercise the server renderer, not the editor, and `@ts-nocheck` covered the dropped prop. Nothing short of loading the page would have caught it.
