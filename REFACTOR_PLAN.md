...
- [x] P6.1 shared/ moves · [x] P6.2 server-only enforced · [x] P6.3 i18n removed · [x] P6.4 store slices *(`store.ts` 964 → 61 ln)* · [ ] P6.5 admin tree split + manual checklist *(views extracted, `MapaFases.tsx` 2 659 → 2 090 ln; container split and checklist still open)* · [x] P6.6 datos split + export→validate round-trip test *(DB-backed merge/replace import still pending: needs `TEST_DATABASE_URL`)* · [x] P6.7 diagnostico split · [x] P6.8 dead code sweep · [ ] **M4 merged**
